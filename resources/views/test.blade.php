...
{{-- Truyền 1 tham số là $questions, $error từ ExaminfoController.php
...
$questions = App\Models\Question::all();
    $save = [];
    function toggle($id) {
        $key = array_search($id, $save);
        if($key == false) {
            $save[] = $id;
        } else {
            array_splice($save, $key);
        }
...
<body style="background-color: rgb(74 222 128)">
    <div class="min-w-[800px] mx-auto">
        <div class="px-4 my-8">
            <h2 class="text-center text-2xl p-4 bg-green-600 text-white rounded-lg ">Bỏ các câu không mong muốn</h2>
        </div>
        {{-- Nếu có lỗi --}}
        @if (isset($error)) 
            {{$error}}
        ...
        <form action="examinfo/store-random-exam" method="post">
            ...
            <div class="flex items-center justify-center mb-4">
                ...
                <br>
            @endforeach
            <button name="ReviewDone" type="submit" #{{}}
